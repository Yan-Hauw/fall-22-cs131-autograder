...
class Interpreter(InterpreterBase):
    ...
    def run(self, program):
        ...
        self.ip = self.func_manager.get_function_info(
            InterpreterBase.MAIN_FUNC
        ).start_ip
        self.return_stack = []
    ...
    def _create_new_environment(self, funcname, args):
        ...
        if formal_params is None:
            super().error(
                ErrorType.NAME_ERROR, f"Unknown function name {funcname}", self.ip
            )

        if len(formal_params.params) != len(args):
            super().error(
                ErrorType.NAME_ERROR,
                f"Mismatched parameter count in call to {funcname}",
                self.ip,
            )
        ...
        for formal, actual in zip(formal_params.params, args):
            ...
            if arg.type() != self.compatible_types[formal_typename]:
                super().error(
                    ErrorType.TYPE_ERROR,
                    f"Mismatched parameter type for {formal_name} in call to {funcname}",
                    self.ip,
                )
        ...
        if value_type.value():
            ...
            for line_num in range(self.ip + 1, len(self.tokenized_program)):
                ...
                if (
                    ...
                    return
                if (
                    tokens[0] == InterpreterBase.ELSE_DEF
                    and self.indents[self.ip] == self.indents[line_num]
                ):
                    self.ip = line_num + 1
    ...
    def _return(self, args):
        ...
        if default_value_type.type() == Type.VOID:
            if args:
                super().error(
                    ErrorType.TYPE_ERROR, "Returning value from void function", self.ip
                )
    ...
    def _define_var(self, args):
        if len(args) < 2:
            super().error(
                ErrorType.SYNTAX_ERROR, "Invalid var definition syntax", self.ip
            )
        for var_name in args[1:]:
            if self.env_manager.create_new_symbol(var_name) != SymbolResult.OK:
                super().error(
                    ErrorType.NAME_ERROR, f"Redefinition of variable {args[1]}", self.ip
                )
    ...
    def _strtoint(self, args):
        ...
        self._set_result(
            Value(Type.INT, int(value_type.value()))
        )  # return always passed back in result
    ...
    def _set_value(self, varname, to_value_type):
        ...
        if value_type == None:
            super().error(
                ErrorType.NAME_ERROR,
                f"Assignment of unknown variable {varname}",
                self.ip,
            )
