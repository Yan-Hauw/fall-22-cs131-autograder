...
class FunctionManager:
    ...
    def _cache_function_parameters_and_return_type(self, tokenized_program):
        ...
        for line_num, line in enumerate(tokenized_program):
            if line and (
                line[0] == InterpreterBase.FUNC_DEF
                or line[0] == InterpreterBase.LAMBDA_DEF
            ):
                # format:  func funcname self.p1:t1 p2:t2 p3:t3 ...
                if line[0] == InterpreterBase.LAMBDA_DEF:
                    func_name = self.create_lambda_name(line_num + 1)
                ...
                func_info = FuncInfo(
                    params, line_num + 1
                )  # function starts executing on line after funcdef
                self.func_cache[func_name] = func_info
                return_type_stack.append(line[-1])

            if line and (
                line[0] == InterpreterBase.ENDFUNC_DEF
                or line[0] == InterpreterBase.ENDLAMBDA_DEF
            ):
            ...
            # for each return
            if (
                reset_after_this_line
            ):  # for each line with a funcend, make sure we know the return type
